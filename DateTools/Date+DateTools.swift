...
func DateToolsLocalizedStrings(_ key: String) -> String {
    return NSLocalizedString(key, tableName: "DateTools", bundle: Bundle(url: Bundle(for: TimePeriodCollection.self).resourceURL!.appendingPathComponent("DateTools.bundle"))!, comment: key)
}

private enum DTDateComponent: UInt {
    case era
    case year
    case month
    case day
...
public extension Date {
     ...
     */
	static var implicitCalendar: Calendar = Calendar(identifier: .gregorian)
	...
	public static func dateWith(year: Int, month: Int, day: Int) -> Date {
        return self.dateWith(year: year, month: month, day: day, hour: 0, minute: 0, second: 0)
    }

	public static func dateWith(dateString: String, formatString: String) -> Date {
        return self.dateWith(dateString: dateString, formatString: formatString, timeZone: TimeZone.current)
    }

	public static func dateWith(dateString: String, formatString: String, timeZone: TimeZone) -> Date {
	...
	private func localizedString(forFormat format: DateAgoFormat, valueType: DateAgo, value: Int) -> String? {
	 ...
	 *  @return Int
	 */
    private func componentFor(date: Date, type component: DTDateComponent, calendar fromCalendar: Calendar?) -> Int {
     ...
     */
	func years(from date: Date, calendar: Calendar = Date.self.implicitCalendar) -> Int {
	 ...
	 */
	func months(from date: Date, calendar: Calendar = Date.self.implicitCalendar) -> Int {
        let earliest = (self as NSDate).earlierDate(date)
        let latest = (earliest == self) ? date : self
        let multiplier = (earliest == self) ? -1 : 1
	 ...
	 */
	func weeks(from date: Date, calendar: Calendar = Date.self.implicitCalendar) -> Int {
        let earliest = (self as NSDate).earlierDate(date)
        let latest = (earliest == self) ? date : self
        let multiplier = (earliest == self) ? -1 : 1
	 ...
	 */
	func days(from date: Date, calendar: Calendar = Date.self.implicitCalendar) -> Int {
        let earliest = (self as NSDate).earlierDate(date)
        let latest = (earliest == self) ? date : self
        let multiplier = (earliest == self) ? -1 : 1
